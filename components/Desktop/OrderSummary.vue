<template>
	<div>
		<div
			class="lg:hidden  w-full h-16 bg-gray-100
            border-t flex items-center justify-between px-3
		...
		<div
			...
			class="bg-white relative w-full lg:w-2/5 lg:fixed lg:inset-y-0
            lg:left-0 lg:block border-r border-gray-400"
		>
			<div class="mx-4 lg:ml-24 lg:mr-20 text-sm py-8 scrolling-auto">
				<div v-for="(order, index) in cart.cartItems" :key="index">
					<div class="w-full flex flex-row-reverse justify-start my-6">
						<div class="relative border border-gray-400 rounded">
							<span
								class="cart-prop w-6 h-6 bg-teal-500 font-bold
                            text-white text-xs rounded-full inline-flex justify-center"
							>
								{{ order.quantity }}
							</span>
							<img
				...
				<div class="my-6 relative">
					<input
						...
						class="border border-gray-400 px-4 h-12 rounded w-full
                    focus:border-black focus:border-2 focus:outline-none appearance-none"
						placeholder="کد کارت تخفیف یا هدیه"
						type="text"
					/>
					<div class="absolute left-0 ml-1 inset-y-center">
						<ThinLine />
						<button ref="apply" class="btn h-10 w-20">
							ثبت
						</button>
					</div>
				</div>
